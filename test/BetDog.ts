...
describe("BetDog", function () {
    async function deployContracts() {
        ...
        const LiquidityPoolERC20 = await ethers.getContractFactory(
            "LiquidityPoolERC20"
        )
        ...
        const pool = await LiquidityPoolERC20.deploy(
            core.address,
            token.address
        )
    ...
    async function testBetting(oddsList: number[], betIndex: number) {
        ...
        console.log(
            "before reserves:",
            (await core.getCondition(conditionId)).reserves
        )
        const beforeSumReserves = (
            await core.getCondition(conditionId)
        ).reserves.reduce((a, b) => a.add(b))
        ...
        console.log(
            "real odds:",
            reward.add(betAmount).mul(multiplier).div(betAmount).toNumber() /
                multiplier
        )
    ...
    describe("Condition", function () {
        it("Should fail if not oracle call", async function () {
            const { core, startTime, endTime, pool } = await loadFixture(
                deployContracts
            )
        ...
        it("Create condition", async function () {
            const { core, oracle, startTime, endTime, pool, token, maker } =
                await loadFixture(deployContracts)
            const multiplier = 1e9
